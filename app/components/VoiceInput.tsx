...
export default function VoiceInput({ onTranscriptChange, transcript }: VoiceInputProps) {
  ...
  const handleStart = useCallback(() => {
    ...
    rec.onerror = (event: SpeechRecognitionErrorEvent) => {
      ...
      setRuntimeError(errorMessages[event.error] || `语音识别错误: ${event.error}`);
    ...
    try {
      ...
      setRuntimeError(`无法启动语音识别: ${err instanceof Error ? err.message : "未知错误"}`);
      setRecognition(null);
    }
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md">
      <h2 className="text-xl font-semibold mb-4 text-gray-800">语音输入</h2>
      ...
      <div className="flex gap-3 mb-4">
        ...
        <button
          ...
          className="py-3 px-6 rounded-lg font-medium bg-gray-200 hover:bg-gray-300 text-gray-700 transition-colors"
        >
          清空
        </button>
      </div>
      ...
      {!hasSpeechSupport && mounted && (
        <div className="mb-4 p-3 bg-yellow-100 text-yellow-700 rounded-md">
          提示：您的浏览器不支持 Web Speech API，请使用 Chrome、Microsoft Edge 或 Safari 浏览器。
        </div>
      )}

      <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
        <p className="text-sm text-blue-700 mb-2">📝 备用方案：如果语音识别不可用，可以直接输入文字</p>
      ...
      <div className="border border-gray-300 rounded-lg p-4 min-h-[100px] bg-gray-50">
        <p className="text-gray-700">
          {transcript || (
            <span className="text-gray-400">识别的文本将显示在这里...</span>
          )}
        </p>
      </div>
    </div>
